...
proc createTimelineRouter*(cfg: Config) =
  router timeline:
    ...
    get "/@name/?@tab?/?":
      ...
      case tab:
        of "followers":
          resp renderMain(renderUserList(await getGraphFollowers(await getUserId(@"name"), getCursor()), prefs, request), request, cfg, prefs)
        of "following":
          resp renderMain(renderUserList(await getGraphFollowing(await getUserId(@"name"), getCursor()), prefs, request), request, cfg, prefs)
        else:
